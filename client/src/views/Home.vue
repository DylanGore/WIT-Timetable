        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <form @submit.prevent="get_timetable">
                    <div class="mb-3">
                        <label for="inputDepatment" class="form-label"
                            >Department</label
                        >
                        <select
                            class="form-select"
                            ...
                            @change="get_courses($event)"
                        >
                            <option
                                v-for="dept in timetableData"
                                :key="dept.code"
                                :value="dept.code"
                            >
                                {{ dept.name }}
                    ...
                    <div class="mb-3" v-if="form.department != null">
                        <label for="inputProgramme" class="form-label"
                            >Programme</label
                        >
                        <select
                            class="form-select"
                        ...
                        >
                            <option
                                v-for="prog in selectedDept.courses"
                                :key="prog.code"
                                :value="prog.code"
                            >
                                {{ prog.name }}
                ...
                <form v-if="timetable.length > 0">
                    <div class="row">
                        <div class="col-sm-6">
                            <label
                                for="inputAddAvailableGroup"
                                class="form-label"
                                >Available groups</label
                            >
                            <select
                                class="form-select"
                                id="inputAddAvailableGroup"
                                size="4"
                                aria-label="Available groups"
                                v-model="groupForm.availableAdd"
                            >
                                <option
                            ...
                            <button
                                ...
                                role="button"
                                @click.prevent="add_wanted_group()"
                            >
                                Add Selected Group
                            </button>
                        </div>
                        <div class="col-sm-6">
                            <label for="inputAddGroup" class="form-label"
                                >Wanted groups</label
                            >
                            <select
                                class="form-select"
                                id="inputAddGroup"
                                size="4"
                                aria-label="Wanted groups"
                                v-model="groupForm.availableRemove"
                            >
                                <option
                                    v-for="group in groupFilters.add"
                                    :key="group"
                                    :value="group"
                                >
                                    {{ group }}
                            ...
                            <button
                                ...
                                role="button"
                                @click.prevent="remove_wanted_group()"
                            >
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr
                                        ...
                                        :key="event.id"
                                        :class="{
                                            'oncampus-lecture': event.type.includes(
                                                'L - On Campus'
                                            ),
                                            'online-lecture': event.type.includes(
                                                'OL - Online'
                                            ),
                                            'online-practical': event.type.includes(
                                                'OP - Online'
                                            ),
                                            'oncampus-practical': event.type.includes(
                                                'P - On Campus'
                                            ),
                                            'oncampus-tutorial': event.type.includes(
                                                'T - On Campus'
                                            ),
                                            'online-tutorial': event.type.includes(
                                                'OT - Online'
                                            ),
...
export default {
    ...
    computed: {
        filteredTimetable: function () {
            this.timetable.forEach((entry) => {
                if (
                    this.groupFilters.add.some(
                        (v) => entry.groups.indexOf(v) !== -1
                    )
                ) {
                    if (entry.id > -1) {
    ...
    methods: {
        get_timetable: function () {
            ...
            this.timetable = [];
            axios
                .post('http://10.0.1.1:5000/get-timetable', this.$data.form)
                .then((res) => {
                    this.loading = false;
                    this.timetable = res.data.timetable;
                    this.timetable.forEach((entry) => {
                        this.allGroups = [
                            ...new Set([...this.allGroups, ...entry.groups]),
                        ];
        ...
        get_courses: function (event) {
            this.selectedDept = this.timetableData.filter(
                (item) => item.code == event.target.value
            )[0];
            return this.timetableData.filter(
                (item) => item.code == event.target.value
            );
        },
        day_classes: function (day) {
            return this.filteredTimetable.filter(
                (event) =>
        ...
        add_wanted_group: function () {
            ...
            this.availableGroups = this.availableGroups.filter(
                (e) => e !== this.groupForm.availableAdd
            );
        },
            ...
            this.timetable.forEach((entry) => {
                if (!entry.ignore) {
                    let start = parse(
                        ...
                        addDays(
                            firstday,
                            this.days.indexOf(entry.day.toLowerCase())
                        )
                    );
